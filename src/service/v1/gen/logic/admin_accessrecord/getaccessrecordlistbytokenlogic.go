...
type GetAccessRecordListByTokenLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewGetAccessRecordListByTokenLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetAccessRecordListByTokenLogic {
	return &GetAccessRecordListByTokenLogic{
...
func (l *GetAccessRecordListByTokenLogic) GetAccessRecordListByToken(req *types.GetAccessRecordListByToken) (resp *types.GetAccessRecordListResp, err error) {
	var accessRecordList []db.AccessRecord
	var count int64

	accessModel := db.NewAccessRecordModel(mysql.MySQLConn)
	accessRecordList, err = accessModel.GetList(l.ctx, req.Token, req.Page, req.PageSize, req.StartTime, req.EndTime, req.TimeType)
	if err != nil {
