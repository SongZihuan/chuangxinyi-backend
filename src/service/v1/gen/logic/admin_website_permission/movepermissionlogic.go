...
func (l *MovePermissionLogic) MovePermission(req *types.MoveReq) (resp *types.RespEmpty, err error) {
	...
	if errors.Is(err, db.ErrNotFound) {
		if req.IsUp {
			return &types.RespEmpty{
				Resp: respmsg.GetRespByMsg(l.ctx, respmsg.CanNotUp, "站点权限在顶部"),
			}, nil
		} else {
			return &types.RespEmpty{
				Resp: respmsg.GetRespByMsg(l.ctx, respmsg.CanNotDown, "站点权限在底部"),
			}, nil
		}
	} else if err != nil {
		return nil, respmsg.MySQLSystemError.WarpQuick(err)
	...
	err = mysql.MySQLConn.TransactCtx(l.ctx, func(ctx context.Context, session sqlx.Session) (err error) {
		permissionModel := db.NewWebsitePolicyModelWithSession(session)
	...
	audit.NewAdminAudit(user.Id, "管理员移动站点权限（%s）成功", permission.Name)
