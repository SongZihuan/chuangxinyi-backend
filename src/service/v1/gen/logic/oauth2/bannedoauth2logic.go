...
func (l *BannedOauth2Logic) BannedOauth2(req *types.BannedOauth2Req) (resp *types.RespEmpty, err error) {
	...
	if !req.AllowLogin {
		err := jwt.DeleteAllWebsiteLoginToken(l.ctx, user.Uid, web.ID)
		if err != nil {
			return nil, respmsg.JWTError.WarpQuick(err)
		}
	}

	audit.NewUserAudit(user.Id, "用户调整站点（%s）访问策略", web.Name)
