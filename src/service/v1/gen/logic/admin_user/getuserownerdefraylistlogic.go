...
type GetUserOwnerDefrayListLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewGetUserOwnerDefrayListLogic(ctx context.Context, svcCtx *svc.ServiceContext) *GetUserOwnerDefrayListLogic {
	return &GetUserOwnerDefrayListLogic{
...
func (l *GetUserOwnerDefrayListLogic) GetUserOwnerDefrayList(req *types.AdminGetOwnerListReq) (resp *types.AdminGetDefrayOwnerListResp, err error) {
	defrayModel := db.NewDefrayModel(mysql.MySQLConn)
	if req.ID == 0 && len(req.UID) == 0 {
		return &types.AdminGetDefrayOwnerListResp{
	...
	defrayList, err := defrayModel.GetListWithOwnerID(l.ctx, user.Id, req.Status, req.Src, req.SupplierID, req.Page, req.PageSize, req.StartTime, req.EndTime, req.TimeType)
	if err != nil {
		return nil, respmsg.MySQLSystemError.WarpQuick(err)
	}
	...
	return &types.AdminGetDefrayOwnerListResp{
		...
		Data: types.AdminGetDefrayOwnerListData{
			Count:  count,
			Defray: respList,
		},
	}, nil
