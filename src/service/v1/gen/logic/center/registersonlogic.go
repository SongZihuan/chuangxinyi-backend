...
func (l *RegisterSonLogic) RegisterSon(req *types.RegisterSonReq) (resp *types.SuccessResp, err error) {
	...
	if !success {
		return &types.SuccessResp{
			Resp: respmsg.GetRespByMsg(l.ctx, respmsg.UserRegisterAgain, "生成用户uuid失败"),
		}, nil
	}
	...
	audit.NewUserAudit(father.Id, "用户注册子用户成功")
